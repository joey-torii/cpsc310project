...
export default class CreateExercise extends Component {
    ...
    // will be called before anything is loaded onto the page
    // the dropdown menu for usernames
    componentDidMount() {
        axios.get('http://localhost:5000/users/')
          .then(response => {
            if (response.data.length > 0) {
              this.setState({
                users: response.data.map(user => user.username, user => user.password), 
    ...
    // TODO: how to check if the passwords are the same
    onChangePassword(e){
        this.setState({
